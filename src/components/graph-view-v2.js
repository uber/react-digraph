...
// GraphViewV2 is in-progress. It's almost the same as orginal one but we are changing the way rendering nodes and edges,
// which will improve the efficiency. If tested thoroughly, we will officially release this version.
class GraphViewV2 extends React.Component<IGraphViewProps, IGraphViewState> {
  ...
  removeEdgeElement(source: string, target: string) {
    ...
    delete this.edgesRef.current[`edge-${id}-container`];
  }
  ...
  handleNodeMove = (position: IPoint, nodeId: string, shiftKey: boolean) => {
    ...
    if (!shiftKey && !draggingEdge) {
      ...
      this.asyncRenderNode(node, false, true);
      ...
      if (selected?.nodes?.size) {
        ...
        if (!foundNodeInSelectedNodes) {
          ...
          selected?.nodes?.forEach((current, key) => {
            ...
            this.asyncRenderNode(selectedNode, false, true);
          });
        }
      }
      ...
      // render new edge
      this.syncRenderEdge(
        { source: nodeId, targetPosition: position },
        false,
        true
      );
  ...
  // $FlowFixMe[signature-verification-failure]
  removeCustomEdge = () => {
    delete this.edgesRef.current['edge-custom-container'];
    delete this.edgesRef.current['edge-custom-custom-container'];
  };
  // $FlowFixMe[signature-verification-failure]
  ...
  handleZoomEnd = () => {
    ...
    this.endDragEdge(undefined, () => {
      ...
      if (
        ...
        this.syncRenderEdge(draggedEdgeCopy, false, true);
        ...
        this.syncRenderEdge(draggedEdge, false, true);
      }
    });
  };
  ...
  asyncRenderNode(
    ...
    this.nodeTimeouts[timeoutId] = requestAnimationFrame(() => {
      this.syncRenderNode(node, nodeMoving, forceUpdate);
    });
  }
  ...
  syncRenderNode(
    ...
    if (nodesMapNode) {
      this.renderConnectedEdgesFromNode(nodesMapNode, nodeMoving, forceUpdate);
    }
  }
  // $FlowFixMe[signature-verification-failure]
  ...
  renderConnectedEdgesFromNode(
    ...
    node.incomingEdges.forEach(edge => {
      this.asyncRenderEdge(edge, nodeMoving, forceUpdate);
    });
  ...
  renderEdge = (
    ...
    if (edgeContainer) {
      if (element) {
        this.edgesRef.current[containerId] = <g id={containerId}>{element}</g>;
      }
  ...
  asyncRenderEdge = (
    ...
    this.edgeTimeouts[timeoutId] = requestAnimationFrame(() => {
      this.syncRenderEdge(edge, nodeMoving, forceUpdate);
    });
  };
  ...
  syncRenderEdge(
    ...
    this.renderEdge(id, element, edge, nodeMoving, forceUpdate);
  }
  // $FlowFixMe[signature-verification-failure]
  renderEdges = () => {
        ...
        >
          ...
          <g className="view" ref={el => (this.view = el)}>
            ...
            <g className="entities" ref={el => (this.entities = el)}>
              ...
              {Object.values(this.edgesRef.current).map(edge => edge)}
            </g>
            {selectionStart && (
              <HighlightArea
