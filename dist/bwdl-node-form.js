            ...
            if (choiceOption === 'Next') {
              ...
              return (
                <div key={choiceOption}>
                  <label>Next:</label> {this.renderNextOptions(choice[choiceOption])}
                </div>
              );
              ...
              return (
                <div key={choiceOption}>
                  <label>{choiceOption}:</label> {this.renderAndObjectArray(choice[choiceOption])}
                </div>
              );
            }
            // text option
            return (<div key={choiceOption}><label>{choiceOption}:</label> {choice[choiceOption]}</div>);
          })}
        </div>
      );
    });
      ...
      return Object.keys(value).map((valueKey) => {
        return (
          <div key={valueKey} className="node-property node-sub-property">
            <label>{valueKey}:</label> {this.renderKey(valueKey, value[valueKey])}
          </div>);
      });
    }
